...
class Player:
    ...
    def answer_puzzle(self, underswitch):
        ...
        if isinstance(room, world.PuzzleTile):
            if not room.solved():
        ...
        elif isinstance(room, world.PuzzleTile2):
            if not room.solved():
                action_input = input("Answer: ")

                if action_input != ("3895"):
                    print("Incorrect Answer, try again")
                else:
                    print("The keypad beeps and glows green, opening the door.")
                    room.puzzle_solved = True
        elif isinstance(room, world.PuzzleTile3):
            if not room.solved():
                action_input = input("Answer: ")

                if action_input != ("2167"):
                    print("Incorrect Answer, try again")
                else:
                    print("The keypad beeps and glows green, opening the door.")
                    room.puzzle_solved = True
    ...
    def trade(self, underswitch):
        room = world.tile_at(self.x, self.y, underswitch)
        room.check_if_trade(self)

    def flee(self, underswitch):
        room = world.tile_at(self.x, self.y, underswitch)
        enemy = room.enemy
        self.run = random.randint(-1, 100)

        if self.run >= 70:
            if self.last_action == 'N':
                self.move_south()
            elif self.last_action == 'E':
                self.move_west()
            elif self.last_action == 'S':
                self.move_north()
            elif self.last_action == 'W':
                self.move_east()

            print ("""
            When the enemy wasn't looking,
            you managed to run away,
            with your life still together.
            Maybe rethink your situation before attempting to fight
            them again
            """)
        else:
            print ("""
            You attempted to run from the enemy,
            but it caught you just at the last second.
            Maybe rethink your desicions and if you can
            beat this foe.
            """)
